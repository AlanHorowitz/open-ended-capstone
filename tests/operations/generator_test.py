...
def make_rows(
    ...
    cursor.execute(
        f"INSERT INTO {table_name} ({column_names}) " f" values {values_substitutions}",
        insert_rows,
    )
...
def test_generate_link_parent(
    ...
    cursor.execute(
        f"select * from {order_line_item_table.get_name()}" " WHERE order_id = 4;"
    )
    ...
    cursor.execute(
        f"select * from {order_line_item_table.get_name()}" " WHERE order_id = 7;"
    )
    assert len(cursor.fetchall()) == 3  # id from batch 1
    cursor.execute(
        f"select * from {order_line_item_table.get_name()}" " WHERE order_id = 42;"
    )
...
def test_set_default(data_generator, customer_table):
    ...
    cursor = data_generator.cur
    cursor.execute(
        f"select customer_referral_type, customer_sex from {customer_table.get_name()}"
    )
    rs = cursor.fetchone()
    assert rs == ["OA", "F"]
...
def test_generate_multiple_xref(
    ...
    count_correct_xref = (
        f" SELECT count(order_line_item_id) from {order_line_item_table.get_name()}"
