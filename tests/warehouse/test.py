...
def test_get_customer_keys_incremental():
    customers = [[1, "cust1"], [2, "cust1"], [3, "cust1"], [4, "cust1"]]
    ...
    pd_cust_address = pd.DataFrame(
        customer_addresses, columns=["address_id", "name", "customer_id"]
    )
...
def test_build_new_dimension_1():
    ...
    assert inserts.at[4, "shipping_city"] == "NJ"
